...
public class OPOperRoleDAOImpl implements OPOperRoleDAO {

    private static final Log log = LogFactory.getLog(OPOperRoleDAOImpl.class.getName());
    ...
    public int insert(OPOperRole obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "INSERT INTO OPOperRole(OperID,RoleID)VALUES(?,?)";
        ...
        {
            String errorCode = ErrorCode.ERR_ADDOPOPERROLEFAIL;
    ...
    public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "UPDATE OPOperRole SET ";
        ...
        SQLLog.logUpdate(log,"OPOperRole",setCols,whereCols);
        ...
        {
            String errorCode = ErrorCode.ERR_MODOPOPERROLEFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(OPOperRole obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "DELETE FROM OPOperRole WHERE OperID = ?  AND RoleID = ? ";

        log.debug("[delete sql]:" + "DELETE FROM OPOperRole WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
        ...
        {
            String errorCode = ErrorCode.ERR_DELOPOPERROLEFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "DELETE FROM OPOperRole WHERE 1=1 " ;
        ...
        SQLLog.logDelete(log,"OPOperRole",whereCols);
    ...
    public boolean isExist(OPOperRole obj) throws EduException
    {
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        ...
        String sql = "SELECT COUNT(*) FROM OPOperRole WHERE OperID = ?  AND RoleID = ? ";

        log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM OPOperRole WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
                        " AND RoleID = " + obj.RoleID );
        ...
        {
            String errorCode = ErrorCode.ERR_QRYOPOPERROLEFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int isExist(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT COUNT(*) FROM OPOperRole WHERE 1=1 " ;
        ...
        SQLLog.logIsExist(log,"OPOperRole",whereCols);
    ...
    public OPOperRole find(OPOperRole obj) throws EduException
    {
        OPOperRole result = null;
        PreparedStatement pstmt = null;
        ResultSet rset = null;

        String sql = "SELECT OperID,RoleID  FROM OPOperRole WHERE OperID = ?  AND RoleID = ? ";

        log.debug("[find sql]:" + "SELECT OperID,RoleID  FROM OPOperRole WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
                        " AND RoleID = " + obj.RoleID );
        ...
        try
        {
            ...
            {
                throw new EduException(ErrorCode.ERR_OPOPERROLENODATA);
            }
        }
        catch(SQLException e)
        {
    ...
    public RowSet select(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT * FROM OPOperRole WHERE 1=1 ";
    ...
    public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT " + fName + " FROM OPOperRole WHERE 1=1 ";

...
public class OPOperAllLimitDAOImpl implements OPOperAllLimitDAO {

    private static final Log log = LogFactory.getLog(OPOperAllLimitDAOImpl.class.getName());
    ...
    public int insert(OPOperAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "INSERT INTO OPOperAllLimit(OperID,LimitTypeID,LimitObject,LimitObjectName)VALUES(?,?,?,?)";
        ...
        {
            String errorCode = ErrorCode.ERR_ADDOPOPERALLLIMITFAIL;
    ...
    public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "UPDATE OPOperAllLimit SET ";
        ...
        SQLLog.logUpdate(log,"OPOperAllLimit",setCols,whereCols);
        ...
        {
            String errorCode = ErrorCode.ERR_MODOPOPERALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(OPOperAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "DELETE FROM OPOperAllLimit WHERE OperID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[delete sql]:" + "DELETE FROM OPOperAllLimit WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
        ...
        {
            String errorCode = ErrorCode.ERR_DELOPOPERALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "DELETE FROM OPOperAllLimit WHERE 1=1 " ;
        ...
        SQLLog.logDelete(log,"OPOperAllLimit",whereCols);
    ...
    public boolean isExist(OPOperAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        ...
        String sql = "SELECT COUNT(*) FROM OPOperAllLimit WHERE OperID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM OPOperAllLimit WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
                        " AND LimitTypeID = " + obj.LimitTypeID +
                        " AND LimitObject = " + StringUtils.addQuote(obj.LimitObject) );
        ...
        {
            String errorCode = ErrorCode.ERR_QRYOPOPERALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int isExist(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT COUNT(*) FROM OPOperAllLimit WHERE 1=1 " ;
        ...
        SQLLog.logIsExist(log,"OPOperAllLimit",whereCols);
    ...
    public OPOperAllLimit find(OPOperAllLimit obj) throws EduException
    {
        OPOperAllLimit result = null;
        PreparedStatement pstmt = null;
        ResultSet rset = null;

        String sql = "SELECT OperID,LimitTypeID,LimitObject,LimitObjectName  FROM OPOperAllLimit WHERE OperID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[find sql]:" + "SELECT OperID,LimitTypeID,LimitObject,LimitObjectName  FROM OPOperAllLimit WHERE OperID = " + StringUtils.addQuote(obj.OperID) +
                        " AND LimitTypeID = " + obj.LimitTypeID +
                        " AND LimitObject = " + StringUtils.addQuote(obj.LimitObject) );
        ...
        try
        {
            ...
            {
                throw new EduException(ErrorCode.ERR_OPOPERALLLIMITNODATA);
            }
        }
        catch(SQLException e)
        {
    ...
    public RowSet select(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT * FROM OPOperAllLimit WHERE 1=1 ";
    ...
    public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT " + fName + " FROM OPOperAllLimit WHERE 1=1 ";

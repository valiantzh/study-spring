...
public class OPFunctionDAOImpl implements OPFunctionDAO {

    private static final Log log = LogFactory.getLog(OPFunctionDAOImpl.class.getName());
    ...
    public int insert(OPFunction obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "INSERT INTO OPFunction(FunctionID,FunctionName,FunctionServiceName,LogFlag,OpenFlag)VALUES(?,?,?,?,?)";
        ...
        try
        {
            ...
            PreparedStmtUtils.setString(pstmt,1,obj.FunctionID);
            PreparedStmtUtils.setString(pstmt,2,obj.FunctionName);
            PreparedStmtUtils.setString(pstmt,3,obj.FunctionServiceName);
            PreparedStmtUtils.setString(pstmt,4,obj.LogFlag);
            PreparedStmtUtils.setString(pstmt,5,obj.OpenFlag);
            result = pstmt.executeUpdate();

        }
        catch(SQLException e)
        {
            String errorCode = ErrorCode.ERR_ADDOPFUNCTIONFAIL;
    ...
    public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "UPDATE OPFunction SET ";
        ...
        SQLLog.logUpdate(log,"OPFunction",setCols,whereCols);
        ...
        {
            String errorCode = ErrorCode.ERR_MODOPFUNCTIONFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(OPFunction obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "DELETE FROM OPFunction WHERE FunctionID = ? ";

        log.debug("[delete sql]:" + "DELETE FROM OPFunction WHERE FunctionID = " + StringUtils.addQuote(obj.FunctionID) );
        ...
        {
            String errorCode = ErrorCode.ERR_DELOPFUNCTIONFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "DELETE FROM OPFunction WHERE 1=1 " ;
        ...
        SQLLog.logDelete(log,"OPFunction",whereCols);
    ...
    public boolean isExist(OPFunction obj) throws EduException
    {
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        ...
        String sql = "SELECT COUNT(*) FROM OPFunction WHERE FunctionID = ? ";

        log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM OPFunction WHERE FunctionID = " + StringUtils.addQuote(obj.FunctionID) );
        ...
        {
            String errorCode = ErrorCode.ERR_QRYOPFUNCTIONFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int isExist(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT COUNT(*) FROM OPFunction WHERE 1=1 " ;
        ...
        SQLLog.logIsExist(log,"OPFunction",whereCols);
    ...
    public OPFunction find(OPFunction obj) throws EduException
    {
        OPFunction result = null;
        PreparedStatement pstmt = null;
        ResultSet rset = null;

        String sql = "SELECT FunctionID,FunctionName,FunctionServiceName,LogFlag,OpenFlag  FROM OPFunction WHERE FunctionID = ? ";

        log.debug("[find sql]:" + "SELECT FunctionID,FunctionName,FunctionServiceName,LogFlag,OpenFlag  FROM OPFunction WHERE FunctionID = " + StringUtils.addQuote(obj.FunctionID) );
        ...
        try
        {
            ...
            if(rset.next() == true)
            {
                result = new OPFunction();
                result.FunctionID = rset.getString(1);
                result.FunctionName = rset.getString(2);
                result.FunctionServiceName = rset.getString(3);
                result.LogFlag = rset.getString(4);
                result.OpenFlag = rset.getString(5);

            }
            else
            {
                throw new EduException(ErrorCode.ERR_OPFUNCTIONNODATA);
            }
        }
        catch(SQLException e)
        {
    ...
    public RowSet select(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT * FROM OPFunction WHERE 1=1 ";
    ...
    public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT " + fName + " FROM OPFunction WHERE 1=1 ";

...
public class OPRoleAllLimitDAOImpl implements OPRoleAllLimitDAO {

    private static final Log log = LogFactory.getLog(OPRoleAllLimitDAOImpl.class.getName());
    ...
    public int insert(OPRoleAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "INSERT INTO OPRoleAllLimit(RoleID,LimitTypeID,LimitObject,LimitObjectName)VALUES(?,?,?,?)";
        ...
        try
        {
            ...
            PreparedStmtUtils.setInt(pstmt,1,obj.RoleID);
            PreparedStmtUtils.setInt(pstmt,2,obj.LimitTypeID);
            PreparedStmtUtils.setString(pstmt,3,obj.LimitObject);
            PreparedStmtUtils.setString(pstmt,4,obj.LimitObjectName);
            result = pstmt.executeUpdate();

        }
        catch(SQLException e)
        {
            String errorCode = ErrorCode.ERR_ADDOPROLEALLLIMITFAIL;
    ...
    public int update(JDBCFieldArray setCols,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "UPDATE OPRoleAllLimit SET ";
        ...
        SQLLog.logUpdate(log,"OPRoleAllLimit",setCols,whereCols);
        ...
        {
            String errorCode = ErrorCode.ERR_MODOPROLEALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(OPRoleAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        int result = 0;

        String sql = "DELETE FROM OPRoleAllLimit WHERE RoleID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[delete sql]:" + "DELETE FROM OPRoleAllLimit WHERE RoleID = " + obj.RoleID +
                        " AND LimitTypeID = " + obj.LimitTypeID +
                        " AND LimitObject = " + StringUtils.addQuote(obj.LimitObject) );
        ...
        {
            String errorCode = ErrorCode.ERR_DELOPROLEALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int delete(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "DELETE FROM OPRoleAllLimit WHERE 1=1 " ;
        ...
        SQLLog.logDelete(log,"OPRoleAllLimit",whereCols);
    ...
    public boolean isExist(OPRoleAllLimit obj) throws EduException
    {
        PreparedStatement pstmt = null;
        ResultSet rset = null;
        ...
        String sql = "SELECT COUNT(*) FROM OPRoleAllLimit WHERE RoleID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[isExist sql]:" + "SELECT COUNT(*) FROM OPRoleAllLimit WHERE RoleID = " + obj.RoleID +
                        " AND LimitTypeID = " + obj.LimitTypeID +
                        " AND LimitObject = " + StringUtils.addQuote(obj.LimitObject) );
        ...
        {
            String errorCode = ErrorCode.ERR_QRYOPROLEALLLIMITFAIL;
            log.error("[DBERROR-("+ errorCode + ")]==>[sqlcode]:"+e.getErrorCode() + " [errmsg]:"+e.getMessage() + " [sql]:"+sql);
            throw new EduException(errorCode);
        }
        finally
    ...
    public int isExist(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT COUNT(*) FROM OPRoleAllLimit WHERE 1=1 " ;
        ...
        SQLLog.logIsExist(log,"OPRoleAllLimit",whereCols);
    ...
    public OPRoleAllLimit find(OPRoleAllLimit obj) throws EduException
    {
        OPRoleAllLimit result = null;
        PreparedStatement pstmt = null;
        ResultSet rset = null;

        String sql = "SELECT RoleID,LimitTypeID,LimitObject,LimitObjectName  FROM OPRoleAllLimit WHERE RoleID = ?  AND LimitTypeID = ?  AND LimitObject = ? ";

        log.debug("[find sql]:" + "SELECT RoleID,LimitTypeID,LimitObject,LimitObjectName  FROM OPRoleAllLimit WHERE RoleID = " + obj.RoleID +
                        " AND LimitTypeID = " + obj.LimitTypeID +
                        " AND LimitObject = " + StringUtils.addQuote(obj.LimitObject) );
        ...
        try
        {
            ...
            if(rset.next() == true)
            {
                result = new OPRoleAllLimit();
                result.RoleID = rset.getInt(1);
                result.LimitTypeID = rset.getInt(2);
                result.LimitObject = rset.getString(3);
                result.LimitObjectName = rset.getString(4);

            }
            else
            {
                throw new EduException(ErrorCode.ERR_OPROLEALLLIMITNODATA);
            }
        }
        catch(SQLException e)
        {
    ...
    public RowSet select(JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT * FROM OPRoleAllLimit WHERE 1=1 ";
    ...
    public String selectFunctions(String fName,JDBCFieldArray whereCols) throws EduException
    {
        ...
        String sql = "SELECT " + fName + " FROM OPRoleAllLimit WHERE 1=1 ";
